...
class MemeManager:
    ...
    async def _chat_with_model(self, prompt:str, system:str, image: Resource, format: Literal["json", "str", "int"] = "str") -> Any:
        """
        与 llm 交互
        """
        ...
        elif model.config.multimodal == False:
        ...
        if format == "int":
            ...
            try:
                ...
                logger.error(f"尝试提取模型回复时出现错误，无法将其转换为JSON格式: {response_text}")
                raise
        else:
            return response_text
        

    async def check_memes_valid(self, session: async_scoped_session, memes: list[Meme]) -> list[Meme]:
        """
        [预检] 检查所有 Memes 有效性
        """
    ...
    async def auto_clean_memes(self, session: async_scoped_session):
        ...
        logger.info(f"已删除 {len(memes_to_delete)} 个 Memes，当前有效 Memes 数量为 {self.all_valid_meme_count}")
        await session.commit()
    ...
    async def add_new_meme(self, session: async_scoped_session, meme_image: Resource):
        ...
        if any([new_meme_hash == meme.hash for meme in self.all_valid_meme]):
            logger.debug(f"检查到此 meme 已存在，停止添加")
            return
        
        if config.enable_security_check:
            check_result:int = await self._chat_with_model("回复纯数字0或1", system=self._generate_prompt_from_template("meme_security_check.jinja2"), image=meme_image, format="int")
            if not check_result:
                logger.warning("此表情包未通过安全检查！已停止添加")

        meme_desc:dict[str, Any] = await self._chat_with_model("以JSON格式生成标签和内容", system=self._generate_prompt_from_template("meme_description.jinja2"), image=meme_image)
        ...
        logger.success(f"已成功添加新的表情包！描述:{new_meme.description}, 标签: {new_meme.tag}")
    ...
    async def query_meme(self, message: Message) -> Optional[Meme]:
        ...
        if config.meme_similarity_method == "levenshtein":
            ...
            raise ValueError(f"未找到要求的相似度匹配算法: {config.meme_similarity_method}")
        ...
        logger.info(f"查询到 Meme: {meme.id}, 标签: {meme.tag}, 描述: {meme.description}")
        return meme